\name{VE.Jk.EB.SW2.RegCoI.Hajek}
\alias{VE.Jk.EB.SW2.RegCoI.Hajek}
\title{The self-weighted two-stage sampling Escobar-Berger (2013) jackknife variance estimator for the estimator of the intercept regression coefficient using the Hajek point estimator}
\description{Computes the self-weighted two-stage sampling Escobar-Berger (2013) jackknife variance estimator for the estimator of the intercept regression coefficient using the Hajek (1971) point estimator.  }
\usage{VE.Jk.EB.SW2.RegCoI.Hajek(VecY.s, VecX.s, VecPk.s, nII, VecPi.s,
...
and \eqn{w_k=1/\pi_k} with \eqn{\pi_k} denoting the inclusion probability of the \eqn{k}-th element in the sample \eqn{s}. If \eqn{s} is a self-weighted two-stage sample, the variance of \eqn{\hat{\alpha}_{Hajek}} can be estimated by the Escobar-Berger (2013) jackknife variance estimator (implemented by the current function):
\deqn{\hat{V}(\hat{\alpha}_{Hajek}) = v_{clu} + v_{obs}}
...
\deqn{\varsigma_{(Ii)}=\frac{n_{I}-1}{n_{I}} (\hat{\alpha}_{Hajek}-\hat{\alpha}_{Hajek(Ii)})}
\deqn{\varepsilon_{(k)}=\frac{n-1}{n} (\hat{\alpha}_{Hajek}-\hat{\alpha}_{Hajek(k)})}
where  \eqn{\hat{\alpha}_{Hajek(Ii)}} and \eqn{\hat{\alpha}_{Hajek(k)}} have the same functional form as \eqn{\hat{\alpha}_{Hajek}} but omitting the \eqn{i}-th cluster and the \eqn{k}-th element, respectively, from the sample \eqn{s}.
...
\seealso{
\code{\link{VE.Jk.EB.SW2.RegCo.Hajek}}\cr\code{\link{VE.Jk.Tukey.RegCoI.Hajek}}\cr\code{\link{VE.Jk.CBS.HT.RegCoI.Hajek}}\cr\code{\link{VE.Jk.CBS.SYG.RegCoI.Hajek}}\cr\code{\link{VE.Jk.B.RegCoI.Hajek}}
}
\examples{
