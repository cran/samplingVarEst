\name{VE.Jk.EB.SW2.Mean.Hajek}
\alias{VE.Jk.EB.SW2.Mean.Hajek}
\title{The self-weighted two-stage sampling Escobar-Berger (2013) jackknife variance estimator for the Hajek (1971) estimator of a mean}
\description{Computes the self-weighted two-stage sampling Escobar-Berger (2013) jackknife variance estimator for the Hajek estimator of a mean.  }
\usage{VE.Jk.EB.SW2.Mean.Hajek(VecY.s, VecPk.s, nII, VecPi.s,
                         VecCluLab.s, VecCluSize.s)}
\arguments{
\item{VecY.s}{vector of the variable of interest; its length is equal to \eqn{n}, the total sample size. Its length has to be the same as the length of \code{VecPk.s}. There must not be any missing value.}
...
where \eqn{w_k=1/\pi_k} and \eqn{\pi_k} denotes the inclusion probability of the \eqn{k}-th element in the sample \eqn{s}. If \eqn{s} is a self-weighted two-stage sample, the variance of \eqn{\hat{\bar{y}}_{Hajek}} can be estimated by the Escobar-Berger (2013) jackknife variance estimator (implemented by the current function):
\deqn{\hat{V}(\hat{\bar{y}}_{Hajek}) = v_{clu} + v_{obs}}
...
\deqn{\varsigma_{(Ii)}=\frac{n_{I}-1}{n_{I}} (\hat{\bar{y}}_{Hajek}-\hat{\bar{y}}_{Hajek(Ii)})}
\deqn{\varepsilon_{(k)}=\frac{n-1}{n} (\hat{\bar{y}}_{Hajek}-\hat{\bar{y}}_{Hajek(k)})}
where  \eqn{\hat{\bar{y}}_{Hajek(Ii)}} and \eqn{\hat{\bar{y}}_{Hajek(k)}} have the same functional form as \eqn{\hat{\bar{y}}_{Hajek}} but omitting the \eqn{i}-th cluster and the \eqn{k}-th element, respectively, from the sample \eqn{s}.
...
\seealso{
\code{\link{VE.Jk.Tukey.Mean.Hajek}}\cr\code{\link{VE.Jk.CBS.HT.Mean.Hajek}}\cr\code{\link{VE.Jk.CBS.SYG.Mean.Hajek}}\cr\code{\link{VE.Jk.B.Mean.Hajek}}
}
\examples{
