#include <math.h>
void VE_Jk_EB_SW2_Corr_Hajek_Clu
(
  ...
  for ( i=0 ; i < *nI ; i++ )
  {
    ...
    for ( k=0 ; k < *n ; k++ )
      if ( VectCLb_s[k] != VectCLb_sI[i] )
      {
        ...
        SUMYY           = SUMYY            + (VecY_s[k]-EstMeanY) * (VecY_s[k]-EstMeanY) / VecPk_s[k] ;
        SUMXX           = SUMXX            + (VecX_s[k]-EstMeanX) * (VecX_s[k]-EstMeanX) / VecPk_s[k] ;
      } ;
    EstCorrYX_iTemp     = SUMYX   / sqrt( SUMYY * SUMXX )                ; //Weights' corrections get canceled.
    PseudosCluTemp      = EstCorrYX_iTemp  - (*EstTheta)                                              ;
